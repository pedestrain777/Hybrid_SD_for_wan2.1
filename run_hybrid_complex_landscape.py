...
说明: 使用 --stages 时，若把 prompt 写在 --stages 后面，须用 parse_intermixed_args（本脚本已启用），
    ...
    parser.add_argument(
        "--spatial-cue",
        default=os.environ.get("WAN_HYBRID_SPATIAL_CUE", "cfg"),
        choices=["cfg", "motion", "warp"],
        help="Hybrid 阶段空间区域选择 cue：cfg / motion / warp。也可用环境变量 WAN_HYBRID_SPATIAL_CUE",
    )
    parser.add_argument(
        "--temporal-top-ratio",
        type=float,
        default=float(os.environ.get("WAN_HYBRID_TEMPORAL_TOP_RATIO", 0.15)),
        help="时间维度选取困难 latent frame 的比例。",
    )
    parser.add_argument(
        "--spatial-top-ratio",
        type=float,
        default=float(os.environ.get("WAN_HYBRID_SPATIAL_TOP_RATIO", 0.08)),
        help="空间维度选取高分 latent 位置的比例。",
    )
    parser.add_argument(
        "--max-cubes",
        type=int,
        default=int(os.environ.get("WAN_HYBRID_MAX_CUBES", 2)),
...
class Args:
    def __init__(self):
        ...
        # Simplified Hybrid cube router
        # 时间维度固定使用 frame-wise latent difference；空间维度可在 cfg/motion/warp 之间切换做实验。
        self.hybrid_routing_mode = "two_stage_cube"
        self.hybrid_temporal_cue = "frame_diff"
        self.hybrid_spatial_cue = SPATIAL_CUE
        self.hybrid_temporal_top_ratio = TEMPORAL_TOP_RATIO
        self.hybrid_max_temporal_segments = 2
        self.hybrid_spatial_top_ratio = SPATIAL_TOP_RATIO
        self.hybrid_max_cubes = MAX_CUBES

        # warp 空间 cue 需要的小范围平移搜索半径；只有 spatial_cue=warp 时才用。
        self.hybrid_warp_max_shift = 2

        # core/outer crop：大模型看 outer 上下文，但只把 core 区域贴回去。
