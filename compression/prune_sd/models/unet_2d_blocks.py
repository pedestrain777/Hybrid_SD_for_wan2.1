...
if diffusers.__version__ in ['0.19.0', '0.20.0']:
    def apply_freeu(
...
def get_down_block(
    ...
    num_selfattn_heads: Optional[int] = None,
    selfattn_out_dim: Optional[int] = None,
    crossattn_out_dim: Optional[int] = None,
    num_crossattn_heads: Optional[int] = None,
    ...
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2D(
            ...
            selfattn_out_dim=selfattn_out_dim,
            crossattn_out_dim=crossattn_out_dim
        )
...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        ...
        upcast_attention: bool = False,
        attention_type: str = "default",
        num_selfattn_heads: int = 1,
        num_crossattn_heads: int = 1,
    ):
        super().__init__()
...
class CrossAttnDownBlock2D(nn.Module):
    def __init__(
        ...
        num_selfattn_heads: int = 1,
        num_crossattn_heads: int = 1,
        selfattn_out_dim: int = None,
        crossattn_out_dim: int = None,
        cross_attention_dim: int = 1280,
        output_scale_factor: float = 1.0,
        downsample_padding: int = 1,
        ...
        self.has_cross_attention = True
        self.num_attention_heads = num_attention_heads
        self.num_selfattn_heads = num_selfattn_heads
        self.num_crossattn_heads = num_crossattn_heads
        if isinstance(transformer_layers_per_block, int):
            transformer_layers_per_block = [transformer_layers_per_block] * num_layers
...
class CrossAttnUpBlock2D(nn.Module):
    def __init__(
        ...
        resnet_pre_norm: bool = True,
        num_attention_heads: int = 1,
        num_selfattn_heads: Tuple[int] = 1,
        num_crossattn_heads: Tuple[int] = 1,
        selfattn_out_dim: Tuple[int] = None,
        crossattn_out_dim: Tuple[int] = None,
        cross_attention_dim: int = 1280,
        output_scale_factor: float = 1.0,
        ...
        for i in range(num_layers):
            ...
            if not dual_cross_attention:
                attentions.append(
                    Transformer2DModel(
                        num_attention_heads=num_attention_heads,
                        num_selfattn_heads=num_selfattn_heads[i],
                        num_crossattn_heads=num_crossattn_heads[i],
                        attention_head_dim=out_channels // num_attention_heads,
                        in_channels=out_channels,
                        num_layers=transformer_layers_per_block[i],
                        cross_attention_dim=cross_attention_dim,
                        ...
                        upcast_attention=upcast_attention,
                        attention_type=attention_type,
                        selfattn_out_dim=None if selfattn_out_dim is None else selfattn_out_dim[i],
                        crossattn_out_dim=None if crossattn_out_dim is None else crossattn_out_dim[i]
                    )
                )
            else:
                attentions.append(
