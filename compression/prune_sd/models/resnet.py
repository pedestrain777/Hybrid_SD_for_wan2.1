...
class ResnetBlock2D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.conv1 = conv_cls(in_channels, self.mid_channels, kernel_size=3, stride=1, padding=1)

        if temb_channels is not None:
            if self.time_embedding_norm == "default":
                self.time_emb_proj = linear_cls(temb_channels, self.mid_channels)
        ...
        self.norm2 = torch.nn.GroupNorm(num_groups=groups_out, num_channels=self.mid_channels, eps=eps, affine=True)

        self.dropout = torch.nn.Dropout(dropout)
        conv_2d_out_channels = conv_2d_out_channels or out_channels
        self.conv2 = conv_cls(self.mid_channels, conv_2d_out_channels, kernel_size=3, stride=1, padding=1)
