...
class CalFlopsPipline(object):
    ...
    def reset_flops_calculate(self):
        ...
        def add_or_reset_attrs(module):
            module.__flops__ = 0
            module.__macs__ = 0
            module.__params__ = sum(p.numel() for p in module.parameters()) # just calculate parameter need training.
    ...
    def print_model_pipline(self, units=None, precision=DEFAULT_PRECISION, print_detailed=True, out_csv=None):
        """Prints the model graph with the calculateing pipline attached to each module.
        ...
        def flops_repr(module):
            ...
            if out_csv is not None and is_leaf_module(module):
                self.dataframes.append({'name': module.name, 'macs': macs, 'macs_ratio': round(100 * macs / total_macs, precision), 'params': params, 'params_ratio': round(100 * params / total_params, precision) if total_params else 0})
