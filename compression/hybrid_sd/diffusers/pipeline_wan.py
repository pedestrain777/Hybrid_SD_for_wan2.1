...
class HybridWanPipeline(WanPipeline):
    ...
    def __call__(
        ...
        guidance_scale: Union[float, List[float], Tuple[float, float]] = 6.0,
    ...
    def _hybrid_call(
        ...
        **kwargs,
        ...
        # 3. Encode prompt
        # Handle guidance_scale as list/tuple or single value
        if isinstance(guidance_scale, (list, tuple)):
            gs_for_cfg_check = max(guidance_scale) if len(guidance_scale) > 0 else 1.0
        else:
            gs_for_cfg_check = guidance_scale
        do_classifier_free_guidance = gs_for_cfg_check > 1.0
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    if use_dynamic_cfg:
                        # Calculate progress (0 -> 1 as denoising progresses)
                        progress = (num_inference_steps - t.item()) / num_inference_steps

                        # Support guidance_scale as [min, max] range or single value
                        if isinstance(guidance_scale, (list, tuple)) and len(guidance_scale) == 2:
                            # Dynamic range: interpolate from max to min as denoising progresses
                            gs_min, gs_max = guidance_scale
                            # Cosine schedule: starts at gs_max, ends at gs_min
                            cosine_factor = (1 - math.cos(math.pi * progress ** 5.0)) / 2
                            self._guidance_scale = gs_max - (gs_max - gs_min) * cosine_factor
                        else:
                            # Original behavior: single value with dynamic scaling
                            gs_value = guidance_scale[0] if isinstance(guidance_scale, (list, tuple)) else guidance_scale
                            self._guidance_scale = 1 + gs_value * (
                                (1 - math.cos(math.pi * progress ** 5.0)) / 2
                            )
                    else:
                        # Dynamic CFG disabled: handle list guidance_scale by using average
                        if isinstance(guidance_scale, (list, tuple)) and len(guidance_scale) == 2:
                            self._guidance_scale = (guidance_scale[0] + guidance_scale[1]) / 2
                        elif isinstance(guidance_scale, (list, tuple)):
                            self._guidance_scale = guidance_scale[0]
                        # else: self._guidance_scale is already a scalar from line 404
