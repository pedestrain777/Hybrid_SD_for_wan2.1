...
class HybridWanPipeline(WanPipeline):
    ...
    def __init__(
        ...
        self.hybrid_roi_config: Dict[str, Any] = {
            ...
            "spatial_cue": "cfg",  # cfg / motion / warp
            "spatial_top_ratio": 0.08,
            "max_cubes": 2,
            "warp_max_shift": 2,

            # Large model sees outer crop but only core region is pasted back.
            "margin_t": 1,
            "margin_h": 4,
            "margin_w": 4,
            "min_crop_t": 1,
    ...
    def _log_router_debug(self, step_idx: int, debug_info: Optional[Dict[str, Any]]):
        ...
        if "router_mode" in debug_info:
            print(
                f"router_mode={debug_info.get('router_mode')} "
                f"temporal_cue={debug_info.get('temporal_cue')} "
                f"spatial_cue={debug_info.get('spatial_cue')} "
                f"temporal_top_ratio={debug_info.get('temporal_top_ratio')} "
                f"spatial_top_ratio={debug_info.get('spatial_top_ratio')} "
                f"max_cubes={debug_info.get('max_cubes')}"
            )
        elif "temporal_pool" in debug_info:
            print(
        ...
        for idx, sd in enumerate(spatial_debug):
            if debug_info.get("router_mode") == "two_stage_cube":
                print(
                    f"  SEG#{idx}: seg={sd['segment']}, seg_score={sd['seg_score']:.6f}, "
                    f"bbox={sd.get('spatial_bbox', 'NA')}, source={sd.get('bbox_source', 'NA')}, "
                    f"num_components={sd.get('num_components', 'NA')}, mask_ratio={sd.get('mask_ratio', 0.0):.4f}"
                )
                continue
