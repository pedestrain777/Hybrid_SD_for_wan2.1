...
class HybridVideoInferencePipeline:
    ...
    def set_pipe_and_generator(self):
        ...
        self.pipe.set_hybrid_roi_config({
            ...
            "routing_mode": getattr(self.args, "hybrid_routing_mode", "two_stage_cube"),
            "temporal_cue": getattr(self.args, "hybrid_temporal_cue", "frame_diff"),
            "temporal_top_ratio": getattr(self.args, "hybrid_temporal_top_ratio", 0.15),
            "max_temporal_segments": getattr(self.args, "hybrid_max_temporal_segments", 2),

            # Spatial cue can be switched among cfg / motion / warp for ablation.
            "spatial_cue": getattr(self.args, "hybrid_spatial_cue", "cfg"),
            "spatial_top_ratio": getattr(self.args, "hybrid_spatial_top_ratio", 0.08),
            "max_cubes": getattr(self.args, "hybrid_max_cubes", 2),
            "max_total_rois": getattr(self.args, "hybrid_max_cubes", 2),  # backward-compatible alias

            # Only used when spatial_cue='warp'.
            "warp_max_shift": getattr(self.args, "hybrid_warp_max_shift", 2),

            # Core/outer crop context.
