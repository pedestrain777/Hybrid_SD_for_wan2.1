...
class HybridVideoInferencePipeline:
    ...
    def set_pipe_and_generator(self):
        ...
        # CRITICAL: Pass args to pipeline for EC-Diff parameters
        self.pipe.args = self.args
        print(f"✓ Pipeline args set: use_ecdiff={getattr(self.args, 'use_ecdiff', False)}")

        # Ensure pipeline execution device matches the main device
        try:
    ...
    def generate(
        ...
        height: int = 480,
        width: int = 720,
        guidance_scale: Union[float, List[float], tuple] = 6.0,
