#!/usr/bin/env python3
"""Observation 1: large-small prediction discrepancy distribution on Wan2.1.
...
Important diagnostic design:
    The small model is NOT allowed to generate its own trajectory. We first run a
    large-only trajectory, save the same latent x_t at selected steps, and then
    feed that exact x_t to both large and small models. This avoids cumulative
    trajectory drift and measures prediction-level discrepancy.
"""
...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="analysis_outputs/obs1_prediction_discrepancy")
...
def plot_distribution(path_pdf: str, path_png: str, bin_centers: np.ndarray, percentages: Dict[int, np.ndarray]):
    ...
    plt.xlabel("Normalized Prediction Difference")
...
def main():
    ...
    tmp_dir_obj = tempfile.TemporaryDirectory(prefix="obs1_latents_", dir=args.output_dir)
    tmp_dir = tmp_dir_obj.name
    entries: List[dict] = []

    print("[Obs1] Pass 1/2: run large-only trajectories and save selected latents/predictions.")
    ...
    print("[Obs1] Pass 2/2: load small model and compare predictions on the same saved latents.")
    small_pipe = create_t2v_pipeline(args.small_task, args.small_ckpt_dir, args.device_id, t5_cpu=args.t5_cpu)
    # Target shape/seq_len should match for Wan2.1 T2V 14B and 1.3B at the same size.
    ...
    # Cache text contexts for each prompt under the small pipeline.
    small_context_cache = {}
    for entry in tqdm(entries, desc="small predictions"):
        ...
        diff = normalized_prediction_difference(pred_large, pred_small)
        ...
        del obj, latent, pred_large, pred_small, diff, diff_np
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    save_csv(os.path.join(args.output_dir, "obs1_prediction_discrepancy.csv"), bin_centers, percentages)
    np.savez(os.path.join(args.output_dir, "obs1_prediction_discrepancy_hist.npz"),
             bin_centers=bin_centers, **{f"step_{s}": percentages[s] for s in selected_steps})
    plot_distribution(
        os.path.join(args.output_dir, "fig_obs1_prediction_discrepancy.pdf"),
        os.path.join(args.output_dir, "fig_obs1_prediction_discrepancy.png"),
        bin_centers,
        percentages,
    )
    ...
    save_json(os.path.join(diag_dir, "summary.json"), {
        "description": "Observation 1 prediction-difference diagnostics. Fractions are percentages of spatiotemporal tokens.",
    ...
    if diagnostic_thresholds:
        ...
        plot_two_metric_by_step(
            ...
            "Token Percentage (%)",
            "Low- and high-discrepancy token fractions",
        )
    plot_metric_errorbar_by_step(
        os.path.join(diag_dir, "fig_debug_mean_discrepancy_by_step.pdf"),
        os.path.join(diag_dir, "fig_debug_mean_discrepancy_by_step.png"),
        per_entry_stats, "mean", "Mean Prediction Difference",
        "Across-prompt mean discrepancy",
    )
    md_lines = [
        "This file is for debugging whether Observation 1 supports the paper story.",
        "",
        "Expected useful pattern: many tokens have small large-small prediction difference, but a non-trivial tail remains.",
        "",
    ...
    write_markdown(os.path.join(diag_dir, "debug_report.md"), "Obs1 Prediction Discrepancy Debug Report", md_lines)

    if args.keep_tmp:
        print(f"[Obs1] Temporary entries kept at: {tmp_dir}")
    else:
        tmp_dir_obj.cleanup()
    print(f"[Obs1] Done. Outputs saved to: {args.output_dir}")
