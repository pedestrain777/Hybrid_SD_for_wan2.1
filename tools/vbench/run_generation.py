...
def generate_one(
    ...
    height: int,
    width: int,
    guidance_scale,  # Can be float, list, or tuple
    fps: int,
    dry_run: bool = False,
    ...
    # Handle guidance_scale: keep as-is if list/tuple, convert to float otherwise
    if isinstance(guidance_scale, (list, tuple)):
        gs_value = guidance_scale  # Pass list/tuple directly for dynamic range
    else:
        gs_value = float(guidance_scale)
...
def main() -> None:
    ...
    if row_start is not None and row_end is not None:
        ...
        rows = rows[row_start:row_end+1]
        logger.info(f"过滤行范围: {row_start}-{row_end}, 从 {original_count} 条记录过滤到 {len(rows)} 条")
    ...
    # Handle guidance_scale: can be float, list [min, max], or tuple (min, max)
    gs_cfg = cfg.get("guidance_scale", 6.0)
    if isinstance(gs_cfg, (list, tuple)):
        guidance_scale = list(gs_cfg)  # Keep as list for dynamic range
    else:
        guidance_scale = float(gs_cfg)

    fps = int(cfg.get("fps", 8))
    use_dynamic_cfg = cfg.get("use_dynamic_cfg", False)

    logger.info(f"生成参数: {num_frames}帧, {height}x{width}, fps={fps}, guidance_scale={guidance_scale}, use_dynamic_cfg={use_dynamic_cfg}")
